 ...
 * 
 * 0 if it is a batch of buy orders, or 1 if it is a batch of sell orders. Note
 * that orders[i] represents a batch of amounti independent orders with the same
 * price and order type. All orders represented by orders[i] will be placed
 ...
 * backlog. Vice versa, if the order is a sell order, you look at the buy order
 * with the largest price in the backlog. If that buy order's price is larger
 * than or equal to the current sell order's price, they will match and be
 * executed, and that buy order will be removed from the backlog. Else, the sell
 * order is added to the backlog. Return the total amount of orders in the
 * backlog after placing all the orders from the input. Since this number can be
