 ...
 * 
 * You may start taking a course at any time if the prerequisites are met. Any
 * number of courses can be taken at the same time. Return the minimum number of
 * months needed to complete all the courses.
 * 
...
public class ParallelCoursesIII {
	...
	public int minimumTime(int n, int[][] relations, int[] time) {
		...
		List<List<Integer>> graph = prepareAdjList(n, preRequisites, relations);

		int[] totalTime = new int[n];
		Queue<Integer> queue = IntStream.range(0, n).filter(idx -> preRequisites[idx] == 0).boxed()
				.collect(Collectors.toCollection(LinkedList::new));
		queue.stream().forEach(v -> totalTime[v] = time[v]);

		while (!queue.isEmpty()) {
			for (int index = queue.size(); index > 0; --index) {
				int src = queue.poll();
				for (int dest : graph.get(src)) {
					totalTime[dest] = Math.max(totalTime[dest], totalTime[src] + time[dest]);
	...
	public static void main(String[] args) {
		int rel[][] = { { 1, 5 }, { 2, 5 }, { 3, 5 }, { 3, 4 }, { 4, 5 } };
		int time[] = { 1, 2, 3, 4, 5 };
		ParallelCoursesIII p = new ParallelCoursesIII();
		System.out.println(p.minimumTime(5, rel, time));
	}
