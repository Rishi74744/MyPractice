 ...
 * Trie storage requirements (Please refer Applications of Trie for more
 * details)
 * 
 * 
 * 
...
public class TrieOperations {
	...
	static class Trie {
		...
		public Trie() {
			this.nodes = new Trie[Constants.TOTAL_ALPHABETS];
		}
	...
	public void insert(String value) {
		Trie prevNode = root;
		for (char node : value.toCharArray()) {
			Trie currentNode = new Trie();
			int index = Character.getNumericValue(node) - 10;
			if (prevNode.getNodes()[index] == null) {
				currentNode.setValue(node);
				prevNode.getNodes()[index] = currentNode;
			} else {
				currentNode = prevNode.getNodes()[index];
			}
			prevNode = currentNode;
		}
		prevNode.setEndOfWord(true);
	}
	...
	public String search(String value) {
		...
		for (char node : value.toCharArray()) {
			int index = Character.getNumericValue(node) - 10;
			if (prevNode.getNodes()[index] != null && prevNode.getNodes()[index].value != null) {
				nodeValue.append(node);
				if (prevNode.getNodes()[index].isEndOfWord() && value.length() == nodeValue.length()) {
	...
	public static void main(String[] args) {
		...
		trieOperations.insert("rishi");
		trieOperations.insert("rishu");
		System.out.println(trieOperations.search("rishi"));
		System.out.println(trieOperations.search("rashi"));
		System.out.println(trieOperations.search("rishu"));
